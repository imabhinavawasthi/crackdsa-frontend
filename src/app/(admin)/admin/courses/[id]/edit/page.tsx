...
export default function EditCoursePage() {
  ...
  // Fetch Course Record for Pre-hydration
  const fetchCourseData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          throw new Error("Course ID not found in database records.");
        }
      ...
      setRawJsonStr(JSON.stringify(loadedCurriculum, null, 2));

      // Expand all sections by default on load
      const exp: Record<string, boolean> = {};
      loadedCurriculum.forEach(sec => {
        exp[sec.id] = true;
      });
      setExpandedSectionIds(exp);

    } catch (err: unknown) {
      console.error("Failed to fetch course details:", err);
      const errMessage = err instanceof Error ? err.message : String(err);
      setFetchingError(errMessage || "Unable to pre-hydrate course details.");
    } finally {
      setFetching(false);
    }
  }, [backendUrl, id, reset]);
  ...
  const onSubmit = async (values: CourseFormValues) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.detail || "Unable to save updates in database.");
      }
      ...
      const errMessage = err instanceof Error ? err.message : String(err);
      setSubmitError(errMessage || "An unexpected error occurred while saving updates.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20 px-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <span className="text-[10px] font-black text-brand-500 uppercase tracking-widest block leading-none mb-1">Editor Console</span>
          <h1 className="text-xl sm:text-2xl font-black text-gray-955 dark:text-white tracking-tight">
            Edit Course Specifications
          </h1>
        </div>
      </div>
