...
export default function EditVideoLecturePage() {
  ...
  // Fetch the specific video lecture detail
  const fetchLectureDetail = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          throw new Error("Video lecture asset not found in database.");
        }
        throw new Error(`Failed to load lecture details: ${res.statusText}`);
      }

      const data: VideoLecture = await res.json();
      
      // Populate primary states
      setTitle(data.title || "");
      ...
      // Populate tags and custom attributes metadata
      ...
      setFetchingError(errMessage || "Unable to fetch this video lecture asset.");
    } finally {
      setLoading(false);
    }
  }, [backendUrl, id]);
  ...
  useEffect(() => {
    if (title) {
      document.title = `Edit: ${title} | CrackDSA`;
    }
  }, [title]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title.trim() || !videoUrl.trim()) {
      setSubmitError("Title and Video URL are required fields.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.detail || "Unable to save asset changes.");
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20 px-4">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <span className="text-[10px] font-black text-brand-500 uppercase tracking-widest block leading-none mb-1">Editor Cockpit</span>
          <h1 className="text-xl sm:text-2xl font-black text-gray-955 dark:text-white tracking-tight">
            Edit Video Lecture Details
          </h1>
        </div>
      </div>
      ...
      {loading ? (
        <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-3xl p-12 text-center space-y-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm font-semibold">Pulling asset records by ID...</p>
        </div>
      ) : fetchingError ? (
        <div className="bg-red-500/5 border border-red-500/10 rounded-2xl p-6 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-500/80 mt-1 font-semibold">{fetchingError}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800/80 rounded-3xl p-6 sm:p-8 space-y-6">
            ...
            {submitSuccess && (
              <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-xs font-semibold text-emerald-600 flex items-center gap-2">
                ...
                <span>Changes saved successfully! Redirecting back to catalog...</span>
              </div>
            )}
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800/80 rounded-3xl p-6 sm:p-8 space-y-6">
            ...
            {customResources.length > 0 && (
              <div className="sm:col-span-2 space-y-4 pt-4 border-t border-gray-100 dark:border-gray-800/80 w-full">
                    ...
                    <div key={index} className="flex flex-col sm:flex-row gap-4 items-end bg-gray-50/50 dark:bg-gray-800/20 p-4 rounded-2xl border border-gray-100 dark:border-gray-850">
                      <div className="flex-1 space-y-1.5 w-full">
                        ...
                        <input 
                          ...
                          className="w-full px-4 py-2.5 rounded-xl border border-gray-250 dark:border-gray-800 bg-transparent text-sm text-gray-955 dark:text-white focus:outline-none focus:ring-1 focus:ring-brand-500 font-medium font-mono"
                        />
                      </div>
                      <div className="flex-2 space-y-1.5 w-full">
                        ...
                        <input 
                          ...
                          className="w-full px-4 py-2.5 rounded-xl border border-gray-250 dark:border-gray-800 bg-transparent text-sm text-gray-955 dark:text-white focus:outline-none focus:ring-1 focus:ring-brand-500 font-medium"
                        />
                      </div>
                      <button
                        type="button"
          ...
          <div className="flex items-center justify-end gap-3.5 border-t border-gray-150 dark:border-gray-800/80 pt-6">
            ...
            >
              {submitting ? "Saving Changes..." : "Save Lecture Details"}
            </Button>
          </div>
