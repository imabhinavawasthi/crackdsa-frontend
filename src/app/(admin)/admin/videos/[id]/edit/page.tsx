...
export default function EditVideoLecturePage() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<VideoFormValues>({
  ...
  const fetchLectureDetail = useCallback(async () => {
    ...
    try {
      ...
      setFetchError(errMessage || "Unable to fetch this video lecture asset.");
    } finally {
      setFetching(false);
    }
  }, [backendUrl, id, reset]);
  ...
  if (fetching) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        <Loader2 size={32} className="animate-spin text-brand-500" />
        <p className="text-gray-500 dark:text-gray-400 text-sm font-semibold">Loading video lecture details...</p>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-semibold mt-0.5">
              Update challenge parameters, stream URLs, and linked resources.
            </p>
          </div>
        </div>
      </div>
      ...
      {submitSuccess && (
        <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-2xl p-4 flex items-start gap-2.5 mb-6">
          ...
          <div className="text-xs font-bold text-emerald-600">
            Video lecture updated successfully! Redirecting...
          </div>
        </div>
      )}
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gray-55 hover:bg-gray-100 dark:bg-gray-800/80 dark:hover:bg-gray-700 text-gray-705 dark:text-gray-300 border border-gray-200 dark:border-gray-700 text-xs font-bold transition-all cursor-pointer"
              >
                <Plus size={13} />
