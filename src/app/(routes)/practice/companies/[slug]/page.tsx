...
export default function PracticeCompaniesDynamicPage() {
  ...
  // Fetch problems list filtered by company slug on mount/slug-change
  useEffect(() => {
    const getCompanyProblems = async () => {
      ...
      try {
        setLoading(true);
        setError(null);
        const data = await fetchCompanyProblems(slug);
        setProblems(data || []);
      } catch (err: any) {
        console.error("Error loading company problems list:", err);
        setError(err.message || "Failed to load company problems.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="max-w-6xl mx-auto px-0 sm:px-4 py-2 sm:py-8 select-none">
      ...
      <div className="mb-6 sm:mb-8">
        <PageHeader
          ...
          subtitle={`Master coding questions frequently asked at ${title}. Solve actual interview problems and review complexity configurations.`}
          accent="brand"
        />
      </div>
