...
import { LogIn, Gift, Compass, BookOpen, Users } from "lucide-react";
...
const AppHeader: React.FC = () => {
  ...
  const navLinks = [
    { name: "My Roadmap", href: "/roadmap", icon: <Compass size={18} /> },
    { name: "Masterclasses", href: "/masterclasses", icon: <BookOpen size={18} /> },
    ...
    <header className="sticky top-0 w-full z-99999 border-b border-gray-200 bg-white/80 backdrop-blur-md dark:border-gray-800 dark:bg-gray-900/80">
      <div className="flex flex-col items-center justify-between grow lg:flex-row lg:px-6">
        <div className="flex items-center justify-between w-full gap-2 px-3 py-3 sm:gap-4 lg:justify-normal lg:px-0 lg:py-4">
          <button
            className="items-center justify-center w-10 h-10 text-gray-500 transition-colors border-gray-200 rounded-lg hover:bg-gray-100 dark:border-gray-800 lg:flex dark:text-gray-400 lg:h-11 lg:w-11 lg:border dark:hover:bg-gray-800"
        ...
        <div className={`${isApplicationMenuOpen ? "flex" : "hidden"} items-center justify-between w-full gap-4 px-5 py-4 lg:flex lg:justify-end lg:px-0 lg:shadow-none`}>
          <div className="flex items-center gap-3">
            ...
            <motion.button 
              ...
              whileTap={{ scale: 0.98 }}
              className="hidden lg:flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-50 dark:bg-brand-500/5 border border-brand-200 dark:border-brand-500/10 text-brand-600 dark:text-brand-400 text-xs font-bold transition-all hover:bg-brand-100 dark:hover:bg-brand-500/10"
            >
              <Gift size={14} />
          ...
          {isLoading ? (
            ...
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Link
                ...
                className="group relative flex items-center justify-center gap-2 overflow-hidden rounded-full bg-gray-900 px-5 py-2.5 text-sm font-bold text-white shadow-xl transition-all hover:shadow-gray-900/10 dark:bg-white dark:text-gray-900"
              >
                <span>Login</span>
